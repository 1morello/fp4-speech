...
class FP4Linear(nn.Module):
    ...
    def forward(self, x):
        ...
        if M >= 128:
            ...
            x_packed, x_scales = quantize_to_fp4_packed(x_padded.float())
            out = torch._scaled_mm(
                x_packed, self.w_packed.t(),
...
def main():
    ...
    count = replace_linears(
        model.ema_model,
        lambda n: "transformer_blocks" in n
    )
    print(f"\nReplaced {count} layers with FP4Linear")
    ...
    os.makedirs("results/f5tts/fp4_w4a4", exist_ok=True)
    ...
    for i, text in enumerate(phrases):
        ...
        try:
            ...
            duration = len(wav) / sr
            sf.write(f"results/f5tts/fp4_w4a4/phrase_{i:02d}.wav", wav, sr)
